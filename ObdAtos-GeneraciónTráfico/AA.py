...
AA = (datos[1:, 1:11])*0.15
tipo_vehiculos = np.zeros([14,3])
P_Carrosxcalle = np.zeros([14,3])
Medias = np.zeros([14,9])
N_vehiculosxhora = []
DatosAA = np.empty((P_Carrosxcalle.shape[0], P_Carrosxcalle.shape[1] + 6), dtype=P_Carrosxcalle.dtype)


for i in range(int(np.floor(len(AA[:]) / 4))):
    for j in range(9):
        Medias[i][j] = (float(np.sum(AA[i * 4:(i + 1) * 4, j])))
...
for i in range(P_Carrosxcalle.shape[0]):
    ...
    DatosAA[i] = np.insert(P_Carrosxcalle[i], 0, extra_element)

DatosAA = np.delete(DatosAA, 4, axis=1)


columnas = ['H_inicio', 'H_fin', 'N_carros', 'Livianos', 'Camiones','P_AA2', 'P_AA1', 'P_AA3']
GuardarenExcel(DatosAA, columnas, 'DatosAA.xlsx')
