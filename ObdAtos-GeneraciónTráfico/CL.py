...
N_vehiculosxhora = []
DatosCL = np.empty((P_Carrosxcalle.shape[0], P_Carrosxcalle.shape[1] + 6), dtype=P_Carrosxcalle.dtype)

for i in range(int(np.floor(len(CL[:])/4))):
    for j in range(9):
        Medias[i][j]=(float(np.sum(CL[i*4:(i+1)*4,j])))
...
for i in range(P_Carrosxcalle.shape[0]):
    ...
    DatosCL[i] = np.insert(P_Carrosxcalle[i], 0, extra_element)

DatosCL = np.delete(DatosCL, 4, axis=1)

columnas = ['H_inicio', 'H_fin', 'N_carros', 'Livianos', 'Camiones','P_CLA2', 'P_CLA1', 'P_CLA3']
GuardarenExcel(DatosCL, columnas, 'DatosCL.xlsx')