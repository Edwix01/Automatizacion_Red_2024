...
const int idr = 35;  // ID del emisor del mensaje
...
void TraCIDemo11p::handlePositionUpdate(cObject* obj) {
    ...
    static int contador = 0; //Contador Necesario para enviar el mensaje una sola vez

    if (mobility->getId() == ide && contador == 0){
        if (simTime() - lastDroveAt >= 7.5) {
            std::cout << "Se envió el mensaje" << std::endl;
            ...
            //Se carga la Informacion a enviar
            ...
            wsm->setNid(currentVehicleId);

            //Carga de los datos - Protocolo de Enrutamiento
            wsm->setPidr(idr);  //Carga ID Receptor
            wsm->setPide(ide);  //Carga ID Emisor
            wsm->setStatus(st); //Verifica si el mensaje ya llego al destinatario
            wsm->setTte(0);     //No se ha utilizado
